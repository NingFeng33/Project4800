async function filterAndSearch() {
  ...
  rows.forEach((row) => {
    ...
    const matchesSearch =
      courseCode.includes(searchQuery) || courseName.includes(searchQuery);
    const matchesProgram =
      selectedProgram === "" || programName === selectedProgram;
...
async function fetchCourses(page = 1) {
  const response = await fetch(
    `/manager/courses?page=${page}&limit=${rowsPerPage}`
  );
...
async function fetchRooms(page = 1) {
  const response = await fetch(
    `/manager/rooms?page=${page}&limit=${rowsPerPage}`
  );
...
async function deleteCourse(courseId) {
  if (confirm("Are you sure you want to delete this course?")) {
    try {
      const response = await fetch(`/manager/courses/${courseId}`, {
        method: "DELETE",
      });
      if (response.ok) {
...
async function deleteRoom(roomId) {
  if (confirm("Are you sure you want to delete this room?")) {
    try {
      const response = await fetch(`/manager/rooms/${roomId}`, {
        method: "DELETE",
      });
      if (response.ok) {
